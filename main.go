...
func (i InfoMessage) String() string {
	// если продолжительность тренировки равна нулю, значит и дистанция равна нулю
	if i.Duration.Minutes() == 0 {
		i.Distance = 0
	}
...
func (w Walking) Calories() float64 {
	...
	sqrSpeed := math.Pow(w.meanSpeed()*KmHInMsec, 2) // средняя скорость в м/с в квадрате
	heightInM := w.Height / CmInM                    // рост в метрах

	calories := ((CaloriesWeightMultiplier*w.Weight + (sqrSpeed/heightInM)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours)
	return calories
}
